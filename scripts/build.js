...
function clean() {
  for (const name of fs.readdirSync(ROOT)) {
    if (['scripts','docs'].includes(name)) continue;
...
function table(rows) {
  return `<div class="table-wrap"><table>${rows.map((r,i)=>`<tr>${r.map(c=> i===0 ? `<th>${c}</th>` : `<td>${c}</td>`).join('')}</tr>`).join('')}</table></div>`;
}
...
<body>
  <header class="site-header">
    <a class="brand" href="/">SalaryDecoded</a>
...
function hero(h1, lead, cta = '') { return `<section class="hero"><div><p class="eyebrow">Understand what your salary is really worth</p><h1>${h1}</h1><p class="lead">${lead}</p>${cta}</div></section>`; }
...
function calcBox(kind) {
  return `<section class="calculator" data-calculator="${kind}">
    <h2>Try the calculator</h2>
    <div class="form-grid">
      <label>Amount <input data-input="amount" type="number" value="${kind === 'hourlyToSalary' ? 25 : 60000}" min="0" step="100"></label>
      <label>Comparison amount <input data-input="compare" type="number" value="75000" min="0" step="100"></label>
...
function home() {
  ...
  return layout({ route:'', title:'SalaryDecoded | Salary and Take-Home Pay Calculators', description:'Understand salary, take-home pay, hourly equivalents, pay rises and tax deductions with clear UK and US salary calculators.', body:
    hero('SalaryDecoded', 'Calculate take-home pay, monthly income, weekly pay and hourly equivalents for useful UK and US salary scenarios.', `<p class="actions">${a('uk/salary-calculator','Start with UK salary')} ${a('us/salary-calculator','Start with US salary')}</p>`) +
